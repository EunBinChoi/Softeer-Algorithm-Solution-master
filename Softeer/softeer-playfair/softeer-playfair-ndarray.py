...
if __name__ == '__main__':
    ...
    key_str2lst = np.array([[None] * 5 for _ in range(5)])
    alphabet = np.array([chr(i) for i in range(65, 91) if chr(i) != 'J'])
    checker = np.array([False for i in range(65, 91) if chr(i) != 'J'])
    ...
    msg_str_pair = np.array([msg_str[i:i + 2] for i in range(0, len(msg_str), 2)])
    ...
    while i < len(msg_str_pair):
        if len(msg_str_pair[i]) == 2:
            if msg_str_pair[i][0] == msg_str_pair[i][1]:
                ...
                msg_str_pair = np.array([left_msg_str[j:j + 2] for j in range(0, len(left_msg_str), 2)])
    ...
    # print(new_msg_str_pair)
    ciphered_msg = np.array([None for _ in range(len(new_msg_str_pair))])
    i = 0
