...
def inputData() -> tuple:
    _n: int = int(input())
    _seq: np.ndarray = np.array([[0] * _n for _ in range(3*_n)])
...
def initGarage(_n: int, _seq: np.ndarray) -> tuple:
    _garage: np.ndarray = np.array([[0] * _n for _ in range(_n)])
    _left_garage: np.ndarray = np.array([[0] * _n for _ in range(2*_n)])
...
def makeGaragewithDisappearBlock(_garage: np.ndarray, _left_garage: np.ndarray, _path: list) -> tuple:
    ...
    _new_garage = _garage.copy()
    _new_left_garage = _left_garage.copy()
    for _col in range(len(_new_garage_stacks)):
        for _row in range(len(_new_garage_stacks[_col])):
...
def makeLocationSet(_n: int, _garage: np.ndarray) -> list:
    _garage_set_color = []
    for i in range(_n):
        for j in range(_n):
...
def calPath(_garage: np.ndarray, _garage_set_color: list, _garage_set_loc: dict) -> dict:
